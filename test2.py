...
# ظرفیت و وضعیت ثبت‌نام رویداد اصفهان
confirmed_users_esfahan = set()
registration_closed_esfahan = False

MAX_CAPACITY_ESFAHAN = None  # الان غیرفعاله، کنترل دستی داری
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'event_kindergarten':
        ...
        await query.edit_message_text(
            "✨️ از ایونت کدوم شهرمون می‌خوای باخبر بشی؟",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if registration_closed_esfahan:
        await update.message.reply_text(
            "❌ ظرفیت رویداد اصفهان بسته شده یا تکمیل شده.\nبرای اطلاع از رویدادهای بعدی با ما همراه باش🌱",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("بازگشت", callback_data='event_kindergarten')],
                [InlineKeyboardButton("پشتیبانی", url=f"https://t.me/{SUPPORT_USERNAME}")],
                [InlineKeyboardButton("ورود به کانال", url=f"https://t.me/{CHANNEL_USERNAME}")]
    ...
    await update.message.reply_text(
        "📝 لطفا قبل از ادامه‌ی مسیر هزینه‌ی رویداد رو براساس تعداد نفرات مشخص کن:\n\n"
        "یک نفر : ۴۵۰ هزارتومان\n"
        "دونفر : ۸۵۵ هزارتومان\n"
        "سه نفر: ۱,۲۶۰ هزارتومان\n"
        "چهار نفر: ۱,۶۶۵ هزارتومان\n"
